...
# TODO
# Need to assemble all the constants in a file.
# Maybe a yml file or use some ros features like rosparam
SAMPLING_FREQ = rospy.get_param('/accel_freq') 
SAMPLING_TIME = rospy.get_param('/explore_sampling_time')
SAMPLING_LENGTH = SAMPLING_FREQ*SAMPLING_TIME
EXPLORE_ANGULAR_VEL = rospy.get_param('/explore_angular_velocity')
...
def explore_structure_dependency():
    ...
    static_accels = collect_initial_accels(SUs, DoFs, SAMPLING_LENGTH)
    accels = collect_accelerometer_values(SUs, DoFs, SAMPLING_LENGTH)
...
def collect_initial_accels(SUs, DoFs, T):
    """
    Parameters
    ----------
    SUs: int
...
def excite_joint(joint_num, angular_vel=EXPLORE_ANGULAR_VEL):
    # TODO
